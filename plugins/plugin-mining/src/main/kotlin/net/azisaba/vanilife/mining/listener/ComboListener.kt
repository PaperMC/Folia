...
import net.azisaba.vanilife.mining.combo.ComboCounterManager
...
internal object ComboListener : Listener {
    ...
    fun onBlockBreak(event: BlockBreakEvent) {
        if (ComboCounter.shouldIncrementCombo(event.block)) {
            val counter = ComboCounterManager.getOrCreate(event.player)
            counter.increment()
        }
    }
    ...
    fun onPlayerQuit(event: PlayerQuitEvent) {
        ComboCounterManager.remove(event.player)
    }
}
