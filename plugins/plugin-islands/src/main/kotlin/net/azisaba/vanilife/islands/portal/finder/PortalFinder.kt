...
class PortalFinder(
    ...
    private suspend fun validateCandidate(
        ...
        for (dx in 0..(candidate.innerWidth + 1)) {
            ...
            if (!testFrameBlockAt(plugin, world, candidate.toBlockPosition(dx, candidate.innerHeight + 1, orientation))) return false
        }
        ...
        for (dy in 0..(candidate.innerHeight + 1)) {
            ...
            if (!testFrameBlockAt(plugin, world, candidate.toBlockPosition(candidate.innerWidth + 1, dy, orientation))) return false
        }
    ...
    private suspend fun testFrameBlockAt(
        plugin: Plugin,
        world: World,
        pos: BlockPosition,
    ): Boolean {
    ...
    private suspend fun getBlockStateAt(
        plugin: Plugin,
        world: World,
        pos: BlockPosition,
    ): BlockState {
